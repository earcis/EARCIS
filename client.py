...
with open('client_config.json') as clientConfigJSONFile:
    ...
    if re.match(r"[^@]+@[^@]+\.[^@]+", clientConfigJSONData['client_id']):
        ...
        print "Error: Client ID is not a standard email address, please check client_config.json."
        exit(0)
    ...
    if (clientConfigJSONData['server_no_check_certificate'] == "true"):
        ...
        print "Warning: You opted to not check TLS server's certificate as per config, it may cause security concerns."
        print
    else:
    ...
    if clientConfigJSONData['server_password'] != "":
        if client_utils.serverpassword_match(clientConfigJSONData['server_password']):
            ...
            print "You have set a password for connecting to this server."
        else:
            print "Error: Server password can only contain numbers and letters, and to a maximum of 32 characters. It is negotiated with clear text on both client and server side."
            exit(0)
    else:
        serverpass = ""
    ...
    if clientConfigJSONData['persistent_client_hash'] == "":
        ...
        if not (re.match("^[A-Za-z0-9_-]+$", clientHashedID) and (len(clientHashedID) == 40)):
            print "Error: Your persistent hash is not accepted, please clear the persistent hash in your config."
            exit(0)
...
while True:
    ...
    if not recipientAddressSet:
        ...
        if re.match("^[A-Za-z0-9_-]+$", recipientAddress) and (len(recipientAddress) == 40):
            ...
            print "To connect to a new server, use /server ServerAddress:Port ServerPassword-omit-if-empty)"
        ...
        while clientNewMessage.startswith('/server'):
            ...
            if len(newServerString) == 3:
                if client_utils.serverpassword_match(newServerString[2]):
                    ...
                    print "Warning: Server password can only contain numbers and letters, and to a maximum of 32 characters. It is negotiated with clear text on both client and server side."
                    break
            try:
